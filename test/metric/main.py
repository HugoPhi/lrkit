...
def generate_random_data(n_samples=1000, n_classes=3):
    ...
    logits = np.random.normal(size=(n_samples, n_classes))
...
def test_compare_with_sklearn():
    ...
    assert np.allclose(metrics.accuracy(), sk_accuracy, atol=1e-4)

    # 精确率（按类）
    sk_precision = precision_score(y_true, y_pred_labels, average=None, zero_division=0)
    assert np.allclose(metrics.precision(), sk_precision, atol=1e-4)

    # 召回率（按类）
    sk_recall = recall_score(y_true, y_pred_labels, average=None, zero_division=0)
    assert np.allclose(metrics.recall(), sk_recall, atol=1e-4)

    # F1（按类）
    sk_f1 = f1_score(y_true, y_pred_labels, average=None, zero_division=0)
    assert np.allclose(metrics.f1(), sk_f1, atol=1e-4)
    ...
    try:
        ...
        assert np.allclose(metrics.auc(), sk_auc, atol=0.05)  # 允许较大误差
        ...
        assert np.allclose(metrics.ap(), sk_ap, atol=0.05)
...
def test_edge_cases():
    ...
    metrics = Metrics(y_true, y_pred)
    assert np.allclose(metrics.accuracy(), 1.0)

    # 全错误预测
    y_pred_wrong = np.roll(y_pred, shift=1, axis=1)  # 错位预测
    metrics_wrong = Metrics(y_true, y_pred_wrong)
    assert np.allclose(metrics_wrong.accuracy(), 0.0)
